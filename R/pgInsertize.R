...
pgInsertizeGeom <- function(data.obj, geom = "geometry", create.table = NULL,
  ...
  if (alter.names) {
    cli::cli_alert_info("Making column names DB-compliant (lowercase and replacing special characters with '_').")
  ...
  if (!is.null(create.table) & !is.null(force.match)) {
    cli::cli_abort("Either create.table or force.match must be null.")
  }
  ...
  if (!is.null(force.match)) {
    if (is.null(conn)) {
      cli::cli_abort("Database connection must be specified when using force.match.")
    }
    ## Name fixing
    ...
    if (is.null(db.cols)) {
      cli::cli_abort("Database table {paste(nt, collapse = '.')} not found.")
    }
    if (is.na(match(geom, db.cols))) {
      cli::cli_abort("Geometry column name not found in database table.")
    }
    rcols          <- colnames(dat)
    ...
    if (!(length(colnames(dat)) == length(rcols)) & !partial.match)  {
      cli::cli_abort("{length(rcols) - length(colnames(dat))} column(s) in data frame are missing in database table ({paste(rcols[is.na(match(rcols, colnames(dat)))],collapse = ", ")}). Rename data frame columns or set partial.match = TRUE to only insert to matching colunns.")

    }

    cli::cli_alert_info("{length(colnames(dat))} out of {length(rcols)} columns of the data frame match database table columns and will be formatted.")
  } else {
  ...
  if (geog) {
    ...
    if (!is.na(proj[1])) {
      ...
      cli::cli_alert_warning("Spatial projection is unknown/unsupported and will be NA in insert object (SRID = 0).")
      if (multi == TRUE) {
        d1 <- apply(df, 1, function(x) paste0("(", open,
                                              toString(paste(gsub("'", "''", x[1:length(colnames(df)) -
                                                                                 1], fixed = TRUE), collapse = "','")), close,
...
pgInsertize <- function(data.obj, create.table = NULL, force.match = NULL,
  ...
  if (!is.data.frame(data.obj)) {
    cli::cli_abort("data.obj must be a data frame.")
  }
  ...
  if (!is.null(force.match)) {
    ...
    if (length(colnames(data.obj)) == 0) {
      cli::cli_abort("No column name matches found in database table.")
    }

    ## stop if colnames do not all match and partial.match = FALSE
    if (!(length(colnames(data.obj)) == length(rcols)) & !partial.match) {
        cli::cli_abort("{length(rcols) - length(colnames(data.obj))} column(s) in data frame are missing in database table ({paste(rcols[is.na(match(rcols,colnames(data.obj)))],collapse = ', ')}). Rename data frame columns or set partial.match = TRUE to only insert to matching columns.")
    }
    cli::cli_alert_info("{length(colnames(data.obj))} out of {length(rcols)} columns of the data frame match database table columns and will be formatted for database insert.")
  } else {
