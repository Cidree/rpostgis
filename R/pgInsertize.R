...
pgInsertizeGeom <- function(data.obj, geom = "geometry", create.table = NULL,
  ...
  # mx <- 1
  # ## If polys (sp) -> eliminate in the future
  ...
  #   silent = TRUE)
  # }
  # # If lines (sp) -> eliminate in the future
  ...
  ## If spatial*dataframe, extract data frame (sp) or sf -> modify in the future
  dat <- data.frame()
  ...
  if (is.null(proj) & suppressPackageStartupMessages(requireNamespace("sf", 
                                                                      quietly = TRUE))) {
    ## Extract proj
  ...
  if (!is.null(create.table)) {
    ...
    if (df.mode) {
      ...
      eval(parse(text = paste0("dat$.rpostgis.geom.",geom," <- '",
                               as.character(sf::st_crs(data.obj)),"'")))
  ...
  open <- "'"
  close <- "',"
  if (multi | geog | as.character(sf::st_geometry_type(data.obj, 
                                                       by_geometry = FALSE)) %in% c("LINESTRING",
                                                                                    "MULTILINESTRING")) {
    ...
    message("Using st_as_text from sf package...")
