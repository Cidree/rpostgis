# pgtablenamefix
#' @title format input for database table names
...
pgtablenamefix<-function(t.nm){
  ...
  if (length(t.nm) == 1 & length(strsplit(t.nm,".",fixed=T)[[1]]) == 2) {
      ...
      stop("Invalid PostgreSQL table name. Schema table names with non-standard characters need to be input as ('schem.a','tabl.e').")
    }
