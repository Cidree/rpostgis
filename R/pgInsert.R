# pgInsert
#' This function takes a take an R \code{sp} object (Spatial* or Spatial*DataFrame), 
#' or a regular data frame, and performs the database insert (and table creation, if specified)
#' on the database. The entire data frame is prepared, but if \code{force.match} 
#' specifies a database table, the R column names are compared to the \code{force.match}
#' column names, and only exact matches are formatted to be inserted. A new 
...
#' @param geom character string. For Spatial* datasets, the name of geometry column in the database table. 
#' (existing or to be created; defaults to 'geom')
#' @param create.table character, schema and table of the PostgreSQL table to create. 
#' Column names will be converted to PostgreSQL-compliant names. Default is NULL (no new table created).
...
pgInsert<-function(conn,data.obj,create.table=NULL,force.match=NULL,geom='geom',new.id=NULL,alter.names=TRUE,encoding=NULL) {
  ...
  geo.classes<-c("SpatialPoints","SpatialPointsDataFrame","SpatialLines",
                 "SpatialLinesDataFrame","SpatialPolygons","SpatialPolygonsDataFrame")
  
  cls<-class(data.obj)[1]
  pgi<-NULL
  if (cls %in% geo.classes) {
    try(pgSRID(data.obj@proj4string,conn,create=TRUE,new.srid=NULL))
    try(pgi<-pgInsertizeGeom(data.obj,geom,create.table,force.match,conn,new.id,alter.names))
  } else if (cls == 'data.frame') {
    try(pgi<-pgInsertize(data.obj,create.table,force.match,conn,new.id,alter.names))
  } else if (cls == 'pgi') {
    pgi<-data.obj
    message('Using previously create pgi object. All arguments except for "conn" and "encoding" will be ignored.')
    if (is.null(pgi$in.tab)) {stop("Table to insert into not specified (in pgi$in.table). Set this and re-run.")}
    #continue
  } else {
  ...
  if (is.null(db.cols)) {stop(paste0("Database table ",gsub('"',"",paste(name,collapse="."))," not found."))}
  ...
  quei<-NULL
  ##send insert query
  try(quei<-dbSendQuery(conn,paste0('Insert into ',name[1],".",name[2],cols2,' VALUES ',values,';')))
