...
#' @param table Name of the schema-qualified table in Postgresql 
#' holding the raster.
#' @param rast Name of the column in 'table' holding the raster object
#' @param proj Can be set to TRUE to automatically take the SRID for 
#' the table in the database. Alternatively, the number of 
...
#' pgGetRast(conn,'schema.tablename',proj=4326,digits=9,
#'          NSEW=c(55,50,17,12))
#' }

pgGetRast <- function(conn, table, rast = "rast", proj = NULL, 
                      digits = 9, NSEW = c(NULL, NULL, NULL, NULL)) {
  
  if (!is.null(NSEW) && is.null(proj)) {
    stop("proj is required if a clipping box is given")
  }
  
  if (isTRUE(proj)) {
    t2 <- strsplit(table, ".", fixed = TRUE)[[1]]
    proj <- dbGetQuery(conn, paste0("select srid from public.raster_columns where r_table_schema = '", 
                                    t2[1], "' AND r_table_name = '", t2[2], "';"))$srid
  }
  ...
  if (!is.null(proj)) {
    ...
    return(rasterFromXYZ(trast, digits = digits))
  }
} 
